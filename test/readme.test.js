...
test("quick start", () => {
  ...
  expect(notes("Fbb4 B##4 Cb5").simplify().sort().toString()).toBe(
    "D#4 B4 C#5",
  );
  expect(scale("Dm").add("C#").transpose("m2").toString()).toBe(
    "Eb F Gb Ab Bb Cb Db D",
  );
  ...
  expect(notes("F Gb A Bb C").supersets().scales()).toContain(
  ...
  expect(
    chord("D4 minor")
      ...
      .exact()
      .chord(),
  ).toBe("minor-major ninth");
});

test("intervals", () => {
  expect(interval("M2").add("m3").isEnharmonic("P4")).toBe(true);
  expect(
    Interval.fromSteps(4, 7).add("M7").simpleTerm().frequencyRatio(),
...
test("notes", () => {
  ...
  expect(
    note("Eb5").transpose("-A5").simplify().toPitchClass().toString(),
  ).toBe("G");
});

test("notelist", () => {
  expect(notes("C4 D#4 Ab4 D5").transpose("-m6").toString()).toBe(
    "E3 F##3 C4 F#4",
  );

  expect(
    notes("Abb E4 C").toPitches(4).simplify().remove("C4").sort().toString(),
  ).toBe("E4 G4");
});
...
test("chords", () => {
  ...
  expect(chord("F minor").transpose("M2").includesAll(["G", "Bb", "D"])).toBe(
    true,
  );
