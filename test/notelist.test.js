import { chord, Note, NoteList, scale } from "#src/index.ts";

test("create note list", () => {
  expect(new NoteList().toString()).toBe("");
  expect(
    new NoteList([new Note("F", "#", 5), new Note("G", "b", 5)]).toString(),
  ).toBe("F#5 Gb5");
...
test("create scale", () => {
  expect(NoteList.fromScale(new Note("C"), "major").toString()).toBe(
    "C D E F G A B",
  );
...
test("transpose note list", () => {
  ...
  expect(NoteList.fromString("C E G").transpose("P5").toString()).toBe("G B D");
});
...
test("sort note list", () => {
  expect(NoteList.fromString("F4 C6 D3 A####5 Bbbb5").sort().toString()).toBe(
    "D3 F4 Bbbb5 A####5 C6",
  );
});
...
test("simplify note list", () => {
  ...
  expect(
    NoteList.fromString("Fb4 C##6 Dbbb3 A####5 Bbbb5").simplify().toString(),
  ).toBe("E4 D6 B2 C#6 G#5");
});
...
test("check if note is in note list", () => {
  ...
  expect(NoteList.fromString("C D E F G").includesAll(["C", "Ebb"])).toBe(
    false,
  );
  expect(NoteList.fromString("C D E F G").includesAll(["C", "Ebb"], true)).toBe(
    true,
  );
});
...
test("check pitches and pitch classes", () => {
  ...
  const mixedNoteList = new NoteList([
    new Note("D", "#"),
    new Note("A", "b", 4),
  ]);
